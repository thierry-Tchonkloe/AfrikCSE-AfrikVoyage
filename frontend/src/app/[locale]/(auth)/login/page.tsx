...
type Translator = ReturnType<typeof useTranslations<"authPages.login">>;
...
export default function LoginPage() {
    const tr = useTranslations("authPages.login");
    ...
    const onSubmit = async (data: FormData): Promise<void> => {
        ...
        try {
            ...
            if (result.type === "partner" && result.partnerUser) {
                toast.success(tCommon("welcomeUser", { name: result.partnerUser.firstName }));
            ...
            if (!result.user) throw new Error(tr("invalidServerResponse"));
            ...
            toast.success(tCommon("welcomeUser", { name: result.user.firstName }));
            ...
            toast.error(getErrorMessage(err, tr("loginError")));
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-[#F4F8FA] flex font-sans antialiased selection:bg-cyan-500/30 overflow-hidden">
                ...
                <div className="w-full max-w-sm mx-auto space-y-8 animate-fadeIn">
                        ...
                        <Link href="/infos" className="flex items-center gap-2.5 group mb-6 self-start sm:self-center">
                            ...
                            <div className="flex flex-col text-left">
                                <span className="text-base font-bold tracking-tight text-slate-900 leading-none mb-0.5">
                                    {tr.rich("afrikWorkspace", { span1: (chunks) => <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-emerald-500">{chunks}</span> })}
                                </span>
                                <span className="text-[9px] font-black tracking-[0.12em] uppercase leading-none text-slate-400">{tr("saasPlatform")}</span>
                            </div>
                        </Link>
                        <h1 className="text-2xl font-bold tracking-tight text-slate-800 self-start sm:self-center">{t("welcomeBack")}</h1>
                        <p className="text-sm text-slate-500 mt-1.5 self-start sm:self-center">{t("loginSubtitle")}</p>
                    </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    {...register("email")}
                                    type="email"
                                    placeholder={tr("emailPlaceholderNom")}
                    ...
                    <div className="flex items-center justify-center gap-5 pt-6 text-[11px] font-medium text-slate-400 border-t border-slate-100">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span>{tr("endEndEncryption")}</span>
                        </div>
                        <div className="w-1 h-1 rounded-full bg-slate-200" />
                        <div className="flex items-center gap-1.5">
                            <CheckCircle2 className="w-4 h-4 text-cyan-500" />
                            <span>{tr("gdprApdpCompliant")}</span>
                        </div>
                    </div>
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-gradient-to-br from-cyan-500/80 via-indigo-500/5 to-emerald-500/10 overflow-hidden">
                ...
                <Image 
                    ...
                    alt={tr("corporateAirportTravelHub")}
                    fill
                    priority
                ...
                <div className="relative z-10 flex flex-col justify-between h-full w-full p-16 text-slate-800">
                    ...
                    <div className="space-y-4">
                        <div className="inline-flex items-center gap-2 bg-cyan-50/80 backdrop-blur-md border border-cyan-100/50 rounded-full px-3.5 py-1.5 shadow-sm">
                            ...
                            <span className="text-xs font-bold uppercase tracking-wider text-cyan-700">{tr("infrastructureTravel")}</span>
                        </div>
                        <h2 className="text-4xl font-black tracking-tight leading-[1.15] max-w-lg text-slate-900">
                            {tr.rich("unifyBusinessTravelEmployee", { span1: (chunks) => <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 via-indigo-600 to-emerald-600">{chunks}</span> })}
                        </h2>
                        <p className="text-slate-600 max-w-md text-sm leading-relaxed font-medium">
                            {tr("manageExpenseBudgetsHandle")}
                        </p>
                    </div>
                    ...
                    <div className="space-y-8">
                        <div className="grid grid-cols-3 gap-6">
                            <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-4 shadow-sm transition-all duration-300 hover:translate-y-[-2px] hover:bg-white">
                                ...
                                <div className="text-[10px] font-bold tracking-wide uppercase text-slate-500 mt-0.5">{tr("countriesCovered")}</div>
                            </div>
                            <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-4 shadow-sm transition-all duration-300 hover:translate-y-[-2px] hover:bg-white">
                                ...
                                <div className="text-[10px] font-bold tracking-wide uppercase text-slate-500 mt-0.5">{tr("travelCosts")}</div>
                            </div>
                            <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-4 shadow-sm transition-all duration-300 hover:translate-y-[-2px] hover:bg-white">
                                ...
                                <div className="text-[10px] font-bold tracking-wide uppercase text-slate-500 mt-0.5">{tr("satisfaction")}</div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white/90 backdrop-blur-md rounded-2xl p-5 border border-slate-100 shadow-md flex flex-col gap-3">
                            <p className="text-sm text-slate-600 font-medium leading-relaxed italic">
                                {tr("implementationAfrikworkspace")}
                            </p>
                            <div className="flex items-center gap-3 border-t border-slate-100 pt-3">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-slate-800">{tr("humanResourcesDepartment")}</p>
                                    <p className="text-[10px] font-semibold text-slate-500">{tr("techafrikGroup")}</p>
                                </div>
                            </div>
                        </div>
                    </div>
