...
type Translator = ReturnType<typeof useTranslations<"company.settings">>;
...
const getSidebarItems = (t: Translator) => ([
    { id: "info",      label: t("companyInformation") },
    { id: "branding",  label: t("branding") },
    { id: "modules",   label: t("modules") },
    { id: "users",     label: t("userManagement") },
    { id: "billing",   label: t("billingPlan") },
]);

export default function CompanySettingsPage() {
    const tr = useTranslations("company.settings");
    ...
    const handleLogoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            toast.error(getErrorMessage(err, t("errorWhileUploadingLogo")));
        } finally {
            e.target.value = "";
        }
    };
    ...
    const handleFaviconChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            toast.success(t("faviconUpdated"));
        } catch (err) {
            toast.error(getErrorMessage(err, t("errorWhileUploadingFavicon")));
        } finally {
            e.target.value = "";
        }
    };
    ...
    return (
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
            <div>
            <h1 className="text-xl font-bold text-gray-900">{t("companyAdministration")}</h1>
            <p className="text-sm text-gray-500">{t("companySettings")}</p>
        ...
        <div className="flex gap-5">
            ...
            {activeSection === "info" && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <input value={settings.name}
                        ...
                        placeholder={t("techcorpInternational")} className={inp} />
                    </Field>
                    <Field label={t("legalName")}>
                    <input value={settings.legalName}
                        onChange={(e) => update("legalName", e.target.value)}
                        placeholder={t("techcorpInternationalSarl")} className={inp} />
                    </Field>
                    <Field label={t("registrationNumber")}>
                    <input value={settings.registrationNumber}
                        onChange={(e) => update("registrationNumber", e.target.value)}
                        placeholder={t("fr2024789456")} className={inp} />
                    </Field>
                    ...
                    <select value={settings.industry}
                        ...
                        className={inp}>
                        {INDUSTRIES.map((i) => <option key={i} value={i}>{optionLabel(i)}</option>)}
                    </select>
                    </Field>
                    ...
                    <Field label={t("primaryContactEmail")}>
                    ...
                    <Field label={t("headquartersAddress")} colSpan>
                    <input value={settings.address}
                        onChange={(e) => update("address", e.target.value)}
                        placeholder={t("text123AvenueDesChamps")}
                        className={inp} />
                    </Field>
                </div>
                </Section>
            ...
            {activeSection === "branding" && (
                <Section title={t("brandingCustomization")}
                desc={tr("customizeLookPlatform")}>
                {/* Logo + Favicon */}
                <div className="grid grid-cols-2 gap-6 mb-5">
                    {/* Logo — fonctionnel (upload Cloudinary) */}
                    <div>
                        <p className="text-xs font-medium text-gray-700 mb-2">{t("companyLogo")}</p>
                        <input
                            ref={logoInputRef}
                            type="file"
                        ...
                        >
                        {settings.logoUrl ? (
                            <img src={settings.logoUrl} alt={t("logo")} className="max-h-12 mx-auto object-contain" />
                        ) : (
                            <>
                            <Upload size={20} className="mx-auto text-gray-400 mb-2" />
                            <p className="text-xs text-gray-500">
                                {t("recommended26056px")}
                            </p>
                            </>
                        )}
                    ...
                    <div>
                        <p className="text-xs font-medium text-gray-700 mb-2">{t("favicon")}</p>
                        <input
                            ref={faviconInputRef}
                            type="file"
                            accept="image/jpeg,image/jpg,image/png,image/svg+xml,image/webp"
                        ...
                        >
                        {settings.faviconUrl ? (
                            <img src={settings.faviconUrl} alt={t("favicon")} className="max-h-12 mx-auto object-contain" />
                        ) : (
                            <>
                            <Upload size={20} className="mx-auto text-gray-400 mb-2" />
                            <p className="text-xs text-gray-500">
            ...
            {activeSection === "modules" && (
                <Section title={t("moduleActivation")}
                desc={tr("enableDisablePlatformModules")}>
                <div className="space-y-4">
                    {[
                    {
                        label: t("afrikvoyageTravelManagement"),
                        desc: t("flightHotelTrainCar"),
                        sub: user?.organization?.hasVoyage ? t("moduleActiveOrganization") : t("inactiveContactSupport"),
                        active: user?.organization?.hasVoyage,
                        color: "#f59e0b",
                        icon: "✈️",
                    },
                    {
                        label: t("afrikcseEmployeeBenefits"),
                        desc: t("manageEmployeeBenefitsCse"),
                        sub: user?.organization?.hasCSE ? t("moduleActiveOrganization") : t("inactiveContactSupport"),
                    ...
                    },
                    {
                        ...
                        desc: t("trackReimburseEmployee"),
                        sub: t("inactiveAvailableProPlan"),
                        active: false,
                        color: "#6b7280",
                        icon: "💰",
                    },
                    {
                        label: t("timeAttendance"),
                        desc: t("trackEmployeeTimeAttendance"),
                        sub: t("inactiveAvailableEnterprise"),
                        active: false,
                        color: "#6b7280",
                        icon: "🕐",
                    },
                    ...
                    <div key={mod.label}
                        ...
                        {mod.active !== undefined && (
                            ...
                            title={mod.active ? t("active") : t("contactSupportActivate")}
                            >
                            <span
            ...
            {activeSection === "users" && (
                <Section title={t("userManagement2")}
                desc={tr("configureRolesPermissions")}>
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900">{t("manageUsers")}</p>
                    <p className="text-xs text-gray-500">
                        {t("addEditManageEmployees")}
                    </p>
                    </div>
                    <button
            ...
            {activeSection === "billing" && (
                <Section title={t("billingSubscription")}
                desc={tr("manageSubscriptionPayment")}>
                <button
