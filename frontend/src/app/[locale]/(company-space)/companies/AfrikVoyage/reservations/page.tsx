...
type Translator = ReturnType<typeof useTranslations<"company.afrikvoyageReservations">>;
...
const getStatusConfig = (tr: Translator): Record<string, { label: string; color: string }> => ({
    PENDING:     { label: tr("pending"), color: "#f59e0b" },
    APPROVED:    { label: tr("approved"),   color: "#3b82f6" },
    IN_PROGRESS: { label: tr("progress"),   color: "#8b5cf6" },
...
const getPaymentConfig = (tr: Translator): Record<string, { label: string; color: string }> => ({
    PENDING:  { label: tr("unpaid"),  color: "#ef4444" },
    PAID:     { label: tr("paid"),      color: "#10b981" },
    FAILED:   { label: tr("failed"),    color: "#ef4444" },
    REFUNDED: { label: tr("refunded"), color: "#6b7280" },
});
...
export default function ReservationsPage() {
    ...
    const tr = useTranslations("company.afrikvoyageReservations");
    const STATUS_CONFIG = useMemo(() => getStatusConfig(tr), [tr]);
    const PAYMENT_CONFIG = useMemo(() => getPaymentConfig(tr), [tr]);
    ...
    const load = useCallback(async () => {
        ...
        toast.error(tr("errorLoadingBookings"));
        } finally {
        setLoading(false);
        }
    ...
    const handleApprove = async (item: Reservation) => {
        ...
        toast.success(tr("bookingApproved"));
        setDetailItem(null);
        load();
        } catch (err) {
    ...
    const handleReject = async (item: Reservation, note: string) => {
        ...
        toast.success(tr("bookingRejected"));
        setRejectItem(null);
        load();
        } catch (err) {
        toast.error(getErrorMessage(err, tr("error")));
    ...
    const handleStatusChange = async (item: Reservation, newStatus: TravelStatus) => {
        ...
        toast.success(tr("statusUpdated"));
    ...
    const handleAssignPartner = async (item: Reservation, partnerId: string) => {
        ...
        toast.success(tr("partnerAssigned"));
        refreshDetail({ ...item, ...updated });
        load();
        } catch (err) {
        toast.error(getErrorMessage(err, tr("error")));
    ...
    const handleUpdatePayment = async (item: Reservation, payload: { paymentStatus?: string; paymentLink?: string }) => {
        ...
        toast.success(tr("paymentUpdated"));
        refreshDetail({ ...item, ...updated });
        load();
        } catch (err) {
        toast.error(getErrorMessage(err, tr("error")));
    ...
    const handleComplete = async (item: Reservation, actualCost: number) => {
        ...
        toast.success(tr("tripClosedActualCost"));
        setDetailItem(null);
        load();
        } catch (err) {
        toast.error(getErrorMessage(err, tr("error")));
    ...
    return (
        ...
        {/* En-tête */}
        <div>
            <h1 className="text-xl font-bold text-gray-900">{tr("travelBookings")}</h1>
            <p className="text-sm text-gray-500">
            {tr("booking", { total, p2: total > 1 ? "s" : "" })}
            </p>
        </div>
        ...
        {stats && (
            ...
            {[
                { label: tr("totalBookings"), value: stats.total.toString(),
                icon: Plane, iconBg: "#eff6ff", iconColor: "#3b82f6" },
                { label: tr("pending"), value: stats.pending.toString(),
                icon: Clock, iconBg: "#fffbeb", iconColor: "#f59e0b" },
                { label: tr("approved2"), value: stats.approved.toString(),
                icon: CheckCircle2, iconBg: "#f0fdf4", iconColor: "#10b981" },
                { label: tr("totalCost"), value: formatCurrency(stats.totalCost, undefined, dateLocale),
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-wrap gap-3 items-end">
            ...
            <div>
                <label className="block text-xs text-gray-500 mb-1">{tr("departureAfter")}</label>
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none" />
            </div>
            <div>
                <label className="block text-xs text-gray-500 mb-1">{tr("departureBefore")}</label>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <table className="w-full">
                ...
                <tr className="border-b border-gray-100 bg-gray-50">
                    {[tr("traveler"), tr("destination"), tr("dates"), tr("estimatedCost"), tr("status"), tr("payment"), tr("partner"), tr("actions")].map((h) => (
                ...
                {loading ? (
                    ...
                    return (
                        ...
                        <td className="px-5 py-3 text-xs text-gray-500">
                            {new Date(r.departureDate).toLocaleDateString(dateLocale)}
                            {" → "}
                            {new Date(r.returnDate).toLocaleDateString(dateLocale)}
                        </td>
                        ...
                        <td className="px-5 py-3">
                            ...
                            {r.status === "PENDING" && (
                                <>
                                <ActionBtn icon={<Check size={14} />} title={tr("approve")}
                                    onClick={() => handleApprove(r)} />
                                <ActionBtn icon={<X size={14} />} title={tr("reject")} danger
                                    onClick={() => openReject(r)} />
                                </>
                            )}
                            <ActionBtn icon={<Eye size={14} />} title={tr("viewDetails")}
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
                <p className="text-xs text-gray-500">{tr("page", { page, totalPages })}</p>
                <div className="flex items-center gap-2">
...
function DetailModal({
    ...
    return (
        <Modal title={tr("booking2", { destination: item.destination })} onClose={onClose}>
        <div className="space-y-4">
            ...
            <div>
                <p className="text-xs text-gray-500">{tr("destination")}</p>
                <p className="font-medium text-gray-900">{item.destination}</p>
            </div>
            <div>
                <p className="text-xs text-gray-500">{tr("urgency")}</p>
                <p className="font-medium text-gray-900">{item.urgency}</p>
            </div>
            <div>
                <p className="text-xs text-gray-500">{tr("departure")}</p>
                <p className="font-medium text-gray-900">{new Date(item.departureDate).toLocaleDateString(dateLocale)}</p>
            </div>
            <div>
                <p className="text-xs text-gray-500">{tr("return")}</p>
                <p className="font-medium text-gray-900">{new Date(item.returnDate).toLocaleDateString(dateLocale)}</p>
            </div>
            <div>
                <p className="text-xs text-gray-500">{tr("estimatedCost")}</p>
                <p className="font-medium text-gray-900">
                {item.estimatedCost != null ? formatCurrency(item.estimatedCost, undefined, dateLocale) : "—"}
                </p>
            </div>
            <div>
                <p className="text-xs text-gray-500">{tr("actualCost")}</p>
                <p className="font-medium text-gray-900">
                {item.actualCost != null ? formatCurrency(item.actualCost, undefined, dateLocale) : "—"}
                </p>
            </div>
            {item.purpose && (
                <div className="col-span-2">
                <p className="text-xs text-gray-500">{tr("reason")}</p>
                <p className="text-gray-700">{item.purpose}</p>
                </div>
            )}
            {item.status === "REJECTED" && item.rejectionNote && (
                <div className="col-span-2">
                <p className="text-xs text-gray-500">{tr("rejectionReason")}</p>
            ...
            <p className="text-xs font-medium text-gray-500 mb-2">{tr("progress2")}</p>
            <div className="flex flex-wrap gap-2">
                ...
                <button disabled={processing} onClick={() => setShowCompleteInput(true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-white text-xs font-medium disabled:opacity-50"
                    style={{ background: "#10b981" }}>
                    <CheckCircle2 size={13} /> {tr("closeEnterActualCost")}
                </button>
                )}
                {item.status === "IN_PROGRESS" && showCompleteInput && (
                ...
                <p className="text-xs text-gray-400">{tr("finalStatusNoAction")}</p>
                )}
            </div>
            </div>
...
function RejectModal({ item, processing, onClose, onConfirm }: {
    ...
    return (
        <Modal title={tr("reject2", { destination: item.destination })} onClose={onClose}>
        <p className="text-sm text-gray-500 mb-3">
            {tr("provideRejectionReason", { firstName: item.requestedBy.firstName, lastName: item.requestedBy.lastName })}
        </p>
        <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3}
            placeholder={tr("rejectionReason2")}
