...
export default function ReportingPage() {
    ...
    const tt = useTranslations("company.afrikvoyageReporting");
    const tr = useTranslations("company.afrikvoyageReporting");
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                toast.error(tr("errorLoadingTravelData"));
            } finally {
                setLoading(false);
            }
        };
    ...
    const STATS = [
        { label: tr("employeesTraveling"), value: travelingNow.length, sub: tr("currentlyTraveling"), icon: Plane, color: "#3b82f6", bg: "#eff6ff" },
        { label: tr("departuresWithin7Days"), value: upcomingDepartures.length, sub: tr("upcomingApprovedTrips"), icon: Calendar, color: "#8b5cf6", bg: "#f5f3ff" },
        { label: tr("returnsWithin7Days"), value: upcomingReturns.length, sub: tr("plannedEndMission"), icon: CheckCircle, color: "#10b981", bg: "#f0fdf4" },
        { label: tr("pendingRequests"), value: stats?.pending ?? 0, sub: (stats?.pending ?? 0) > 0 ? tr("requiresValidation") : tr("nonePending"), icon: AlertTriangle, color: "#f59e0b", bg: "#fffbeb" },
    ];

    const alerts: { title: string; desc: string; color: string; bg: string; icon: typeof Plane; href?: string }[] = [];
    if ((stats?.pending ?? 0) > 0) {
        alerts.push({
            title: tr("travelRequestSPending", { pending: stats!.pending }),
            desc: tr("theseRequestsRequire"),
    ...
    if (departuresIn48h.length > 0) {
        alerts.push({
            title: tr("departureSScheduledWithin", { length: departuresIn48h.length }),
            desc: tr("checkTheseEmployeesHave"),
    ...
    if (returnsIn48h.length > 0) {
        alerts.push({
            title: tr("returnSFromMission", { length: returnsIn48h.length }),
            desc: tr("rememberSchedulePostMission"),
    ...
    if (alerts.length === 0) {
        alerts.push({
            title: tr("noActiveAlerts"),
            desc: tr("allOngoingTripsUnder"),
            color: "#10b981", bg: "#f0fdf4", icon: CheckCircle,
        });
    }

    const QUICK_ACTIONS = [
        { label: tr("approvals"), href: "/companies/AfrikVoyage/approbations", icon: CheckCircle },
        { label: tr("bookings"), href: "/companies/AfrikVoyage/reservations", icon: Plane },
        { label: tr("expenseReports"), href: "/companies/AfrikVoyage/frais", icon: Receipt },
        { label: tr("dashboard"), href: "/companies/AfrikVoyage/dashboard", icon: LayoutDashboard },
    ];
    ...
    return (
        ...
        {/* En-tête */}
        <div>
            <h1 className="text-xl font-bold text-gray-900">{tr("dutyCareMonitoring")}</h1>
            <p className="text-sm text-gray-500">
            {tr("realTimeTrackingEmployees")}
            </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <h3 className="font-semibold text-gray-900 mb-4">{tr("alertsReminders")}</h3>
            ...
            <h3 className="font-semibold text-gray-900 mb-3">{tr("quickActions")}</h3>
            <div className="space-y-2">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <h3 className="font-semibold text-gray-900">{tr("employeesCurrentlyTraveling")}</h3>
            <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder={tr("searchEmployeeDestination")}
                className="text-xs px-3 py-1.5 border border-gray-200 rounded-lg outline-none"
            />
            </div>
            {filteredTravelers.length === 0 ? (
            <p className="text-sm text-gray-400 py-6 text-center">
                {tr("noEmployeeCurrentlyTraveling")}
            </p>
            ) : (
            <div className="overflow-x-auto">
                ...
                <thead>
                    ...
                    {[tr("employee"), tr("destination"), tr("period"), tr("status"), tr("action")].map((h) => (
                ...
                <tbody>
                    ...
                    let statusLabel = tr("returns", { p1: formatDate(t.returnDate, dateLocale) });
                    let statusColor = "#10b981";
                    if (days < 0) { statusLabel = tt("returnOverdue"); statusColor = "#ef4444"; }
                    else if (days === 0) { statusLabel = tt("returningToday"); statusColor = "#f59e0b"; }
                    else if (days <= 2) { statusLabel = tt("returningD", { days }); statusColor = "#f59e0b"; }
                    ...
                    return (
                        ...
                        <td className="py-2.5 text-xs text-gray-600">
                            {formatDate(t.departureDate, dateLocale)} → {formatDate(t.returnDate, dateLocale)}
                        </td>
                        <td className="py-2.5">
