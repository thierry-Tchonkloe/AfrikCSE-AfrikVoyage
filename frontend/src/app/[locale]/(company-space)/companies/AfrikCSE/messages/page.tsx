...
export default function MessagesPage() {
    ...
    const t = useTranslations("company.afrikcseMessages");
    const { user } = useAuth();
    ...
    const loadConversations = useCallback(async () => {
        ...
        } catch { toast.error(t("errorLoadingConversations")); }
    }, [isSuperAdmin]);
    ...
    const loadMessages = useCallback(async () => {
        ...
        } catch { toast.error(t("errorLoadingMessages")); }
    }, [activeConv]);
    ...
    const getConvName = (conv: Conversation): string => {
        if (isSuperAdmin) return conv.organization?.name ?? t("unknown");
        // Affiche le nom du Super Admin
        const sa = conv.participants.find((p) => p.user.role === "SUPER_ADMIN");
        return sa ? `${sa.user.firstName} ${sa.user.lastName}` : t("support");
    };
    ...
    return (
        ...
        <div className="w-64 flex flex-col border-r border-gray-200 shrink-0">
            ...
            <h2 className="font-bold text-gray-900">{t("messages")}</h2>
            {isSuperAdmin && (
