...
type Translator = ReturnType<typeof useTranslations<"company.afrikcseEmployes">>;
...
export default function EmployesPage() {
    const tr = useTranslations("company.afrikcseEmployes");
    const ROLE_LABELS = useMemo(() => getRoleLabels(tr), [tr]);
    const t = useTranslations("company.afrikcseEmployes");
    ...
    const handleActivate = async (userId: string) => {
        ...
        toast.success(t("employeeReactivated"));
        load();
        } catch { toast.error(t("error")); }
    };
    ...
    return (
        ...
        <div className="flex items-center justify-between">
            <div>
            <h1 className="text-xl font-bold text-gray-900">{t("employeeManagement")}</h1>
            <p className="text-sm text-gray-500">
                {t("manageCompanysEmployees")}
            </p>
            </div>
            <button
        ...
        {stats && (
            ...
            {[
                { label: t("activeEmployees"),   value: stats.active,     icon: Users,     color: "#3b82f6", bg: "#eff6ff", sub: stats.newThisMonth > 0 ? t("month", { newThisMonth: stats.newThisMonth }) : t("noNewOnesMonth") },
                { label: t("departments"),      value: stats.departments, icon: Building2, color: "#10b981", bg: "#f0fdf4", sub: "—" },
                { label: t("managers"),          value: stats.managers,    icon: Briefcase, color: "#f59e0b", bg: "#fffbeb", sub: "—" },
                { label: t("suspendedAccounts"), value: stats.suspended,   icon: UserX,     color: "#ef4444", bg: "#fef2f2", sub: "—" },
            ].map((s) => (
                <div key={s.label}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-wrap gap-3">
            ...
            <input
                ...
                placeholder={t("searchNameEmailEmployee")}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <thead>
                ...
                {[t("employee"), t("department"), t("role"), t("status"), t("actions")].map((h) => (
            ...
            <tbody>
                ...
                employees.map((emp) => (
                    ...
                    <td className="px-5 py-3">
                        ...
                        <Btn icon={<Eye size={14} />} title={t("view")} onClick={() => {}} />
                        <Btn icon={<Pencil size={14} />} title={t("edit")} onClick={() => {}} />
                        {emp.user.isActive ? (
                            <Btn icon={<Pause size={14} />} title={t("suspend")}
                            onClick={() => handleDeactivate(emp.user.id)} />
                        ) : (
                            <Btn icon={<Play size={14} />} title={t("reactivate")}
                            onClick={() => handleActivate(emp.user.id)} />
                        )}
                        <Btn icon={<Trash2 size={14} />} title={t("delete")}
            ...
            <p className="text-xs text-gray-500">
                {t("showing1Employees", { length: employees.length, total })}
            </p>
            <div className="flex gap-1 items-center">
                <PagBtn label={t("previous")} onClick={() => setPage(p => Math.max(1, p - 1))}
                disabled={page <= 1} />
                ...
                <PagBtn label={t("next")} onClick={() => setPage(p => Math.min(totalPages, p + 1))}
        ...
        {showAdd && (
            ...
            <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
                <h3 className="font-bold text-gray-900 mb-4">{t("addEmployee")}</h3>
                ...
                <Link href="/companies/users?action=new"
