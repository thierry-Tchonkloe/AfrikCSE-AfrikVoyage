...
type Translator = ReturnType<typeof useTranslations<"company.afrikcseFaq">>;

const getStatusLabels = (t: Translator): Record<FaqStatus, { label: string; color: string; bg: string }> => ({
    DRAFT:     { label: t("draft"),  color: "#6b7280", bg: "#f3f4f6" },
    PUBLISHED: { label: t("published"),     color: "#059669", bg: "#d1fae5" },
    ARCHIVED:  { label: t("archived"),   color: "#d97706", bg: "#fef3c7" },
});

const EMPTY_FORM: FaqInput = { question: "", answer: "", category: "", order: 0, status: "DRAFT" };

export default function AdminFaqPage() {
    const t = useTranslations("company.afrikcseFaq");
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            toast.error(t("errorWhileLoading"));
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h1 className="text-xl font-bold text-gray-900">{t("faqManagement")}</h1>
                    <p className="text-sm text-gray-500">
                        {t("publishedDraft", { published, p2: published > 1 ? "s" : "", drafts })}
                    </p>
                </div>
                <button
                    onClick={openCreate}
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-100 text-left">
                                ...
                                <th className="px-4 py-3 text-xs font-medium text-gray-500">{t("question")}</th>
                                <th className="px-4 py-3 text-xs font-medium text-gray-500 hidden sm:table-cell">{t("category")}</th>
                                <th className="px-4 py-3 text-xs font-medium text-gray-500 hidden md:table-cell">{t("order")}</th>
                                <th className="px-4 py-3 text-xs font-medium text-gray-500">{t("status")}</th>
                                <th className="px-4 py-3 text-xs font-medium text-gray-500 text-right">{t("actions")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {entries.map((entry) => {
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1 justify-end">
                                                <button
                                                    ...
                                                    title={entry.status === "PUBLISHED" ? t("unpublish") : t("publish")}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-xl max-h-[90vh] overflow-y-auto">
                        <h3 className="font-bold text-gray-900 mb-5">
                            {editing ? t("editEntry") : t("newFaqEntry")}
                        </h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">{t("question2")}</label>
                                <input
                                    value={form.question}
                                    onChange={(e) => setForm({ ...form, question: e.target.value })}
                                    placeholder={t("howCanISubmit")}
                                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">{t("answer")}</label>
                                <textarea
                                    value={form.answer}
                                    onChange={(e) => setForm({ ...form, answer: e.target.value })}
                                    rows={5}
                                    placeholder={t("detailedAnswer")}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder={t("travelBenefits")}
                                        className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-700 mb-1">{t("displayOrder")}</label>
                                    <input
                                        type="number"
                                        value={form.order ?? 0}
