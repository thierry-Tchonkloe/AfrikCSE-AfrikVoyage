...
type Translator = ReturnType<typeof useTranslations<"company.integrations">>;
...
const getStatusConfig = (t: Translator): Record<string, { label: string; color: string; icon: string }> => ({
    SUCCESS: { label: t("success"), color: "#10b981", icon: "✓" },
    PARTIAL: { label: t("partial"), color: "#f59e0b", icon: "⚠" },
    FAILED:  { label: t("failed"),  color: "#ef4444", icon: "✗" },
});

const getTypeLabels = (tr: Translator): Record<string, string> => ({
    HR: tr("hrHris"),
    ACCOUNTING: tr("accounting"),
});

const getFieldMappingOptions = (t: Translator) => ([
    { key: "email",      label: t("emailUniqueIdentifier"), options: ["email", "username", "employee_id"] },
    { key: "department", label: t("departmentService"),      options: ["department", "division", "team"] },
    { key: "job_title",  label: t("jobTitleRole"),            options: ["job_title", "position", "role"] },
]);
...
function formatDate(value: string | null, t: Translator, locale: string) {
...
export default function IntegrationsPage() {
    ...
    const tr = useTranslations("company.integrations");
    const TYPE_LABELS = useMemo(() => getTypeLabels(tr), [tr]);
    const t = useTranslations("company.integrations");
    const optionLabel = useOptionLabel();
    const STATUS_CONFIG = useMemo(() => getStatusConfig(t), [t]);
    const FIELD_MAPPING_OPTIONS = useMemo(() => getFieldMappingOptions(t), [t]);
    ...
    const loadIntegrations = async () => {
        try {
            ...
            toast.error(t("errorLoadingIntegrations"));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreate = async () => {
        if (!form.name.trim()) {
            toast.error(t("toolNameRequired"));
            return;
        }
        setCreating(true);
        try {
    ...
    const handleToggleActive = async () => {
        ...
        try {
            ...
            toast.success(updated.isActive ? t("integrationActivated") : t("integrationDeactivated"));
        } catch (err) {
    ...
    const handleTestConnection = async () => {
        ...
        try {
            ...
            toast.success(t("validConfigurationSimulation", { name: selected.name }));
        } catch (err) {
            toast.error(getErrorMessage(err, t("connectionFailed")));
        } finally {
            setTesting(false);
        }
    };
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            if (log.status === "SUCCESS") {
                ...
                updateSelected(updated);
                toast.success(t("simulatedSynchronization"));
            } else {
                toast.error(t("synchronizationFailedCheck"));
            }
        } catch (err) {
            toast.error(getErrorMessage(err, t("errorDuringSynchronization")));
        } finally {
            setSyncing(false);
        }
    };
    ...
    const handleGenerateKey = async () => {
        ...
        try {
            ...
            toast.error(getErrorMessage(err, t("errorWhileGeneratingKey")));
        } finally {
    ...
    const handleRevokeKey = async () => {
        ...
        try {
            ...
            updateSelected(updated);
            toast.success(t("apiKeyRevokedAll"));
        } catch (err) {
            toast.error(getErrorMessage(err, t("errorWhileRevoking")));
        } finally {
            setKeyActionLoading(false);
        }
    };
    ...
    return (
        ...
        <div className="flex items-start justify-between">
            <div>
            <h1 className="text-xl font-bold text-gray-900">{t("hrAccountingIntegrationsApi")}</h1>
            <p className="text-sm text-gray-500">
                {t("connectHrToolsAutomatically")}
            </p>
            </div>
        ...
        {(showCreateForm || integrations.length === 0) && (
            ...
            <h3 className="font-semibold text-gray-900">
                {integrations.length === 0 ? t("connectFirstTool") : t("newIntegration")}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                <label className="block text-xs text-gray-500 mb-1">{t("toolName")}</label>
                <input
                    value={form.name}
                    onChange={(e) => setForm({ ...form, name: e.target.value })}
                    placeholder={t("sapSuccessfactorsWorkdaySage")}
                ...
                <select
                    ...
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none"
                >
                    <option value="HR">{t("hrHris")}</option>
                    <option value="ACCOUNTING">{t("accounting")}</option>
                </select>
                </div>
                <div>
                <label className="block text-xs text-gray-500 mb-1">{t("apiKeyOptional")}</label>
                <input
                    value={form.apiKey}
                ...
                <div>
                <label className="block text-xs text-gray-500 mb-1">{t("webhookUrlOptional")}</label>
                <input
        ...
        {selected && (
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                ...
                <span>⚠️</span>
                <span>
                    {t("simulationModeConnectionTest")}
                </span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-3 border border-gray-100 rounded-xl">
                    <p className="text-xs text-gray-500 mb-1">{t("connectedTool")}</p>
                    <p className="text-sm font-semibold text-gray-900">{selected.name}</p>
                    <p className="text-xs text-gray-400">{TYPE_LABELS[selected.type] ?? selected.type}</p>
                </div>
                <div className="p-3 border border-gray-100 rounded-xl">
                    <p className="text-xs text-gray-500 mb-1">{t("lastSynchronization")}</p>
                    <p className="text-sm font-semibold text-gray-900">{formatDate(selected.lastSyncAt, t, dateLocale)}</p>
                </div>
                <div className="p-3 border border-gray-100 rounded-xl">
                    ...
                    {selected.isActive ? t("connected") : t("inactive")}
                    </span>
                </div>
                </div>
                <div className="flex gap-2">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                ...
                <div>
                <p className="text-xs text-gray-500 mb-2">{t("currentApiKey")}</p>
                {selected.apiKey ? (
                ...
                {selected.apiKey && (
                    ...
                    >
                    <Trash2 size={14} /> {t("revokeKey")}
                    </button>
                )}
                </div>
                <div className="p-3 rounded-xl text-xs"
                style={{ background: "#fffbeb", color: "#92400e" }}>
                ⚠️ <strong>{t("securityWarning")}</strong> {t("keepApiKeysSecret")}
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                ...
                <div>
                <label className="block text-xs text-gray-500 mb-1">{t("webhookUrl")}</label>
                <input
                    value={webhookUrl}
                ...
                <div className="space-y-3">
                    <p className="text-xs font-semibold text-gray-700">{t("fieldMapping")}</p>
                ...
                {/* Options sync */}
                <div className="space-y-3">
                    <p className="text-xs font-semibold text-gray-700">{t("synchronizationOptions")}</p>
                    {[
                    { key: "autoSync",    label: t("automaticSynchronization"), desc: t("automaticallySynchronizes") },
                    { key: "autoCreate",  label: t("automaticCreation"),        desc: t("automaticallyCreatesNew") },
                    { key: "autoDisable", label: t("automaticDeactivation"),   desc: t("deactivatesEmployeesWhoHave") },
                    ].map((opt) => (
                    <div key={opt.key}
                    ...
                    <div>
                    <p className="text-xs font-medium text-gray-900 mb-1">{t("synchronizationFrequency")}</p>
                    <select
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 flex items-center justify-between">
                <div>
                <h3 className="font-semibold text-gray-900 flex items-center gap-2">
                    {t("manualSynchronization")}
                </h3>
                <p className="text-xs text-gray-500 mt-1">
                    {t("launchImmediate")}
                </p>
                <p className="text-xs text-gray-400 mt-0.5">
                    {t("lastSynchronization2", { p1: formatDate(selected.lastSyncAt, t, dateLocale) })}
                </p>
                </div>
                <button
                onClick={handleSync}
            ...
            <div className="bg-white rounded-xl border border-gray-200">
                ...
                <h3 className="font-semibold text-gray-900">{t("historyLogs")}</h3>
                </div>
                <div className="overflow-x-auto">
                {logsLoading ? (
                    <div className="p-5 text-sm text-gray-400">{t("loading")}</div>
                ) : logs.length === 0 ? (
                    <div className="p-5 text-sm text-gray-400">{t("noSynchronizationMoment")}</div>
                ) : (
                    <table className="w-full">
                    <thead>
                        <tr className="border-b border-gray-100 bg-gray-50">
                        {[t("date"), t("type"), t("employeesCreated"), t("updated"), t("errors"), t("status")].map((h) => (
                    ...
                    <tbody>
                        ...
                        return (
                            ...
                            <td className="px-5 py-3 text-xs text-gray-600">{formatDate(log.createdAt, t, dateLocale)}</td>
                            <td className="px-5 py-3 text-xs text-gray-600">
                                {log.type === "AUTOMATIC" ? t("automatic") : t("manual")}
                            </td>
