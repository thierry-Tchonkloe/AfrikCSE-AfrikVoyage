...
type Translator = ReturnType<typeof useTranslations<"company.users">>;
...
const getRoleConfig = (tr: Translator): Record<string, { label: string; color: string }> => ({
    ADMIN:            { label: tr("admin"),    color: "#8b5cf6" },
    MANAGER:          { label: tr("manager"),  color: "#3b82f6" },
    RH:               { label: tr("hr"),       color: "#10b981" },
    FINANCE:          { label: tr("finance"),  color: "#f59e0b" },
    EMPLOYE:          { label: tr("employee"),  color: "#6b7280" },
});

// ── Schéma création utilisateur ────────────────────────

const getCreateSchema = (tr: Translator) => (z.object({
    firstName:  z.string().min(1, tr("firstNameRequired")),
    lastName:   z.string().min(1, tr("lastNameRequired")),
...
type CreateForm = z.infer<ReturnType<typeof getCreateSchema>>;
...
function UsersContent() {
    ...
    const tr = useTranslations("company.users");
    const ROLE_CONFIG = useMemo(() => getRoleConfig(tr), [tr]);
    ...
    const load = useCallback(async () => {
        ...
        toast.error(tr("errorLoadingUsers"));
        } finally {
        setLoading(false);
        }
    ...
    const handleDeactivate = async (u: UserItem) => {
        if (!confirm(tr("deactivate", { firstName: u.firstName, lastName: u.lastName }))) return;
        try {
        await companyService.deactivateUser(u.id);
        toast.success(tr("userDeactivated"));
        load();
        } catch {
    ...
    const handleActivate = async (u: UserItem) => {
        ...
        toast.success(tr("userReactivated"));
        load();
        } catch {
        toast.error(tr("error"));
        }
    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
            <h1 className="text-xl font-bold text-gray-900">{tr("users")}</h1>
            <p className="text-sm text-gray-500">
                {tr("userOrganization", { total, p2: total > 1 ? "s" : "" })}
            </p>
            </div>
            <button
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <table className="w-full">
                ...
                <tr className="border-b border-gray-100 bg-gray-50">
                    {[tr("user"), tr("role"), tr("department"), tr("lastLogin"), tr("status"), tr("actions")].map((h) => (
...
function CreateUserModal({ onClose, onSuccess,}: {
    ...
    const optionLabel = useOptionLabel();
    const tr = useTranslations("company.users");
    const createSchema = useMemo(() => getCreateSchema(tr), [tr]);
    ...
    const onSubmit = async (data: CreateForm): Promise<void> => {
        ...
        toast.success(tr("userCreatedInvitationEmail"));
        onSuccess();
        } catch (err) {
        toast.error(getErrorMessage(err, tr("creationError")));
        } finally {
        setLoading(false);
        }
    };
    ...
    return (
        <Modal title={tr("addUser")} onClose={onClose}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
            <Field label={tr("firstName")} error={errors.firstName?.message}>
                <input {...register("firstName")} placeholder={tr("john")} className={inp} />
            </Field>
            <Field label={tr("lastName")} error={errors.lastName?.message}>
                <input {...register("lastName")} placeholder={tr("doe")} className={inp} />
            </Field>
            <Field label={tr("email")} error={errors.email?.message} colSpan>
                <input {...register("email")} type="email" placeholder={tr("emailPlaceholderJean")} className={inp} />
            </Field>
            <Field label={tr("role2")} error={errors.role?.message}>
                <select {...register("role")} className={inp}>
                <option value="EMPLOYE">{tr("employee")}</option>
            ...
            <Field label={tr("position")}>
                <input {...register("jobTitle")} placeholder={tr("projectManager")} className={inp} />
            </Field>
            <Field label={tr("department")} colSpan>
                <select {...register("department")} className={inp}>
                <option value="">{tr("select")}</option>
                {DEPARTMENTS.map((d) => <option key={d} value={d}>{optionLabel(d)}</option>)}
                </select>
            </Field>
            <Field label={tr("phone")} colSpan>
                <input {...register("phone")} placeholder={tr("text229XxXxXx")} className={inp} />
            </Field>
            </div>
...
function EditUserModal({ user, onClose, onSuccess,}: {
    ...
    return (
        <Modal title={tr("edit2", { firstName: user.firstName, lastName: user.lastName })} onClose={onClose}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
            <Field label={tr("firstName2")}>
                <input {...register("firstName")} className={inp} />
            </Field>
            <Field label={tr("lastName2")}>
                <input {...register("lastName")} className={inp} />
            </Field>
            <Field label={tr("position")} colSpan>
                <input {...register("jobTitle")} placeholder={tr("projectManager")} className={inp} />
            </Field>
            <Field label={tr("department")} colSpan>
                <select {...register("department")} className={inp}>
...
function RoleModal({ user, processing, onClose, onConfirm,}: {
    ...
    return (
        <Modal title={tr("role3", { firstName: user.firstName, lastName: user.lastName })} onClose={onClose}>
        <p className="text-sm text-gray-500 mb-4">
            {tr("selectNewRoleUser")}
        </p>
