...
export default function CarteMembre() {
    ...
    const t = useTranslations("employee.carteMembre");
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                toast.error(t("errorLoadingMembershipCard"));
            } finally {
                setLoading(false);
            }
        })();
    ...
    const handleDownload = () => {
        ...
        if (!svgEl) { toast.error(t("qrCodeNotFound")); return; }
    ...
    if (loading) {
        return (
            <div className="space-y-5">
                <div>
                    <h1 className="text-xl font-bold text-gray-900">{t("membershipCard")}</h1>
                    <p className="text-sm text-gray-500">{t("digitalCseCard")}</p>
                </div>
    ...
    return (
        <div className="space-y-6">
                ...
                >
                    ...
                    <div className="px-6 pb-4 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-teal-300 text-xs mt-0.5">
                                {t("memberSince", { p1: new Date(card.memberSince).toLocaleDateString(dateLocale, {
                    ...
                    <div className="bg-white/10 px-6 py-4 flex items-center justify-between gap-4">
                        ...
                        <div className="flex-1 text-right">
                            <p className="text-teal-200 text-xs">{t("scanQrCode")}</p>
                            <p className="text-teal-100 text-xs">{t("verifyIdentity")}</p>
                            <p className="text-white font-mono text-xs mt-2 opacity-60">
                                {t("afrikcse", { p1: new Date().getFullYear() })}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 max-w-sm mx-auto">
                <h3 className="font-semibold text-gray-900 mb-3 text-sm">{t("information")}</h3>
                <div className="space-y-2">
                    {[
                        { label: t("memberNumber"), value: card.memberId },
                        { label: t("organization"),      value: card.orgName },
                        { label: t("memberSince2"),     value: new Date(card.memberSince).toLocaleDateString(dateLocale, { day: "2-digit", month: "long", year: "numeric" }) },
                    ].map((row) => (
