...
type Translator = ReturnType<typeof useTranslations<"employee.avantagesDetail">>;
...
const getUrgencyOptions = (t: Translator) => ([
    { value: "LOW",    label: t("low"),  color: "#10b981" },
    { value: "MEDIUM", label: t("normal"),  color: "#f59e0b" },
    { value: "HIGH",   label: t("urgent"),  color: "#ef4444" },
]);

export default function AvantageDetailPage() {
    const dateLocale = useDateLocale();
    const t = useTranslations("employee.avantagesDetail");
    ...
    const openModal = () => {
        if (!item) return;
        if (!matchingCategory) {
            toast.error(t("noCseBenefitCategory"));
            return;
        }
        if (matchingCategory.remainingAmount <= 0) {
            toast.error(t("ceilingCategoryHasBeen"));
            return;
        }
    ...
    const handleFile = async (file: File) => {
        ...
        try {
            ...
            toast.error(t("errorWhileUploading"));
            setReceiptFile(null);
        } finally {
    ...
    const handleSubmit = async () => {
        ...
        if (!amount || amount <= 0) { toast.error(t("invalidAmount")); return; }
        if (amount > matchingCategory.remainingAmount) {
            toast.error(t("amountExceedsRemaining", { p1: matchingCategory.remainingAmount.toLocaleString() }));
            return;
        }
        setSubmitting(true);
        try {
            await employeeService.submitBenefitRequest({
                ...
                amount,
                description: submitDesc || t("benefitUsage", { title: item?.title ?? "" }),
                urgency: submitUrgency,
                receipts: receiptUrl ? [receiptUrl] : [],
            });
            toast.success(t("requestSubmittedAwaiting"));
            closeModal();
        } catch (err) {
            toast.error(getErrorMessage(err, t("errorWhileSubmitting")));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleGenerateTicket = async () => {
        ...
        if (item.requiresFamilyMember && !selectedMember) {
            toast.error(t("selectFamilyMemberOffer"));
            return;
        }
        ...
        try {
            ...
            toast.error(err?.response?.data?.message ?? t("unableGenerateTicket"));
        } finally {
    ...
    if (notFound || !item) {
        return (
            <div className="max-w-3xl text-center py-16 text-gray-500">
                ...
                <p className="font-medium">{t("benefitNotFound")}</p>
    ...
    return (
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
            <button onClick={() => router.push("/employes/avantages")}
            className="hover:underline">{t("benefits")}</button>
            <span>/</span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="flex items-center gap-3">
                ...
                {item.offerType && (
                    <span className="ml-1 text-xs font-medium px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600">
                        {item.offerType === "VOUCHER" ? t("voucher") : item.offerType === "BOOKING" ? t("booking") : t("promoCode")}
                    </span>
                )}
                <h1 className="text-lg font-bold text-gray-900 mt-1">{item.title}</h1>
                {item.partner && (
                    <p className="text-xs text-gray-500">{t("by", { name: item.partner.name })}</p>
                )}
                {item.validUntil && (
                    <p className="text-xs text-gray-500">
                        {t("validUntil", { p1: new Date(item.validUntil).toLocaleDateString(dateLocale) })}
                    </p>
                )}
                </div>
            </div>
            <button
                onClick={() => toast.info(t("linkCopied"))}
            ...
            <div className="lg:col-span-2 space-y-4">
                ...
                {item.description && (
                <div>
                    <h3 className="font-semibold text-gray-900 mb-2">{t("offerDescription")}</h3>
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <>
                        ...
                        {t("totalPriceXof", { p1: totalPrice.toLocaleString() })}
                        </p>
                        <p className="text-sm font-semibold" style={{ color: "#10b981" }}>
                        {t("saveXof", { p1: item.companyPrice.toLocaleString(), subsidyPct: item.subsidyPct })}
                        </p>
                    </>
                    )}
                </div>
                <div className="space-y-1.5 text-sm border-t border-gray-100 pt-3">
                    {[
                    { label: t("cseSubsidy"),   value: `${item.companyPrice.toLocaleString()} XOF`, color: "#0f766e" },
                    { label: t("contribution"), value: `${item.employeePrice.toLocaleString()} XOF`, color: "#111", bold: true },
                    ].map((row) => (
                ...
                {item.requiresTicket ? (
                    ...
                    >
                        <TicketIcon size={15} /> {t("getTicket")}
                    </button>
                ) : (
                    <button
                        onClick={openModal}
                ...
                {item.stock !== null && item.stock !== undefined && (
                    <p className="text-xs text-center text-gray-400">
                        {item.stock > 0
                            ? t("spotAvailable", { stock: item.stock, p2: item.stock > 1 ? "s" : "" })
                            : t("outStock")
                        }
                    </p>
                )}
                ...
                {item.requiresFamilyMember && (
                    <div className="border border-indigo-100 bg-indigo-50 rounded-xl p-3 flex items-center gap-2 text-xs text-indigo-700">
                        <Users size={14} />
                        <span>{t("offerRequiresFamilyMember")}</span>
                    </div>
                )}
            </div>
            </div>
                    ...
                    {generatedTicket ? (
                        ...
                        <div className="text-center">
                            <div className="flex items-center justify-center gap-2 mb-2">
                                ...
                                <h3 className="font-bold text-gray-900">{t("ticketReady")}</h3>
                            </div>
                            ...
                            {generatedTicket.expiresAt && (
                                <p className="text-xs text-gray-400 mb-4">
                                    {t("expires", { p1: new Date(generatedTicket.expiresAt).toLocaleDateString(dateLocale) })}
                                </p>
                            )}
        ...
        {showModal && matchingCategory && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
                    <div className="flex justify-between items-center mb-5">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">
                                {t("availableCeiling")} <strong>{matchingCategory.remainingAmount.toLocaleString()} XOF</strong>
                            </p>
                        </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">{t("requestedAmountXof")}</label>
                            <input type="number" value={submitAmount}
                                onChange={(e) => setSubmitAmount(e.target.value)}
                                placeholder={t("max", { p1: matchingCategory.remainingAmount.toLocaleString() })}
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">{t("descriptionJustification")}</label>
                            <textarea value={submitDesc} onChange={(e) => setSubmitDesc(e.target.value)}
                                placeholder={t("benefitUsage", { title: item.title })}
                                rows={3}
                                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm outline-none resize-none focus:border-teal-400"
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-gray-700 mb-2">{t("priority")}</label>
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">{t("supportingDocumentOptional")}</label>
                            {receiptFile ? (
                                <div className="flex items-center gap-3 p-2.5 border border-gray-200 rounded-lg">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-400">{uploadingReceipt ? t("sending") : receiptFile.size}</p>
                                    </div>
                                ...
                                <button type="button"
                                    ...
                                    <Upload size={14} /> {t("uploadSupportingDocumentPdf")}
                                </button>
                            )}
