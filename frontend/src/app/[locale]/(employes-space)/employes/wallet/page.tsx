...
type Translator = ReturnType<typeof useTranslations<"employee.wallet">>;

// ── Wallet helpers ────────────────────────────────────────────────────────────

const getEntryLabels = (tr: Translator): Record<WalletEntryType, string> => ({
    ALLOCATION:        tr("allocation"),
    DEBIT:             tr("debit"),
    SUBSIDY_CREDIT:    tr("subsidyCredited"),
    CASHBACK_CREDIT:   tr("cashbackCredited"),
    CASHBACK_REVERSAL: tr("cashbackReversed"),
    REFUND:            tr("refund"),
    EXPIRY:            tr("expiry"),
    REWARD_CREDIT:     tr("rewardCredited"),
});
...
function formatAmount(raw: string, type: WalletEntryType, locale: string): string {
    const n    = parseFloat(raw);
    const sign = POSITIVE_TYPES.includes(type) ? "+" : "";
    return `${sign}${n.toLocaleString(locale, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} XOF`;
}

// ── Cashback helpers ──────────────────────────────────────────────────────────

const getCashbackStatus = (tr: Translator): Record<CashbackTransaction["status"], { label: string; color: string; icon: React.ElementType }> => ({
    CREDITED:        { label: tr("credited"),       color: "text-green-600 bg-green-50 dark:bg-green-900/30",   icon: CheckCircle2 },
    CALCULATED:      { label: tr("calculated"),        color: "text-blue-600 bg-blue-50 dark:bg-blue-900/30",     icon: CheckCircle2 },
    PENDING_REVIEW:  { label: tr("underReview"),color: "text-amber-600 bg-amber-50 dark:bg-amber-900/30", icon: Clock        },
    REJECTED:        { label: tr("rejected"),         color: "text-red-600 bg-red-50 dark:bg-red-900/30",       icon: AlertTriangle},
});
...
export default function WalletEmployePage() {
    ...
    const tr = useTranslations("employee.wallet");
    const ENTRY_LABELS = useMemo(() => getEntryLabels(tr), [tr]);
    const CASHBACK_STATUS = useMemo(() => getCashbackStatus(tr), [tr]);
    ...
    return (
        <div className="p-6 space-y-6 max-w-2xl mx-auto">
            ...
            <div className="rounded-2xl bg-gradient-to-br from-blue-600 to-blue-700 text-white p-6 flex items-center justify-between shadow-lg">
                <div>
                    <p className="text-sm opacity-80 mb-1">{tr("availableBalance")}</p>
                    {loading && tab === "wallet" ? (
                        <div className="h-9 w-32 bg-white/20 rounded-lg animate-pulse" />
                    ) : (
                        <p className="text-3xl font-bold tracking-tight">
                            {balanceNum !== null
                                ? balanceNum.toLocaleString(dateLocale, { minimumFractionDigits: 0, maximumFractionDigits: 2 })
                                : "—"}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={loadWallet} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition" title={tr("refresh")}>
            ...
            <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
                {([
                    { id: "wallet",   label: tr("transactions"),    icon: Wallet },
                    { id: "cashback", label: tr("cashback"),      icon: Gift   },
            ...
            {tab === "wallet" && (
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                        <h2 className="font-semibold text-gray-800 dark:text-gray-100">{tr("transactionHistory")}</h2>
                        <p className="text-xs text-gray-500 mt-0.5">{tr("transaction", { entryTotal, p2: entryTotal !== 1 ? "s" : "" })}</p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-16 text-gray-400">
                            ...
                            <p className="text-sm">{tr("noTransactionsMoment")}</p>
                        </div>
                    ) : (
                            ...
                            {entries.map((e) => (
                                <li key={e.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                                        ...
                                        <p className="text-xs text-gray-400">
                                            ...
                                            {e.expiresAt && (
                                                <span className="ml-2 text-amber-500">
                                                    {tr("expires", { p1: new Date(e.expiresAt).toLocaleDateString(dateLocale) })}
                                                </span>
                                            )}
                                        </p>
                                    </div>
                                    <p className={`text-sm font-semibold tabular-nums shrink-0 ${
                                        ...
                                        {formatAmount(e.amount, e.type, dateLocale)}
                                    </p>
                                </li>
                            ))}
                        </ul>
                    ...
                    {!loading && entries.length < entryTotal && (
                        <div className="px-5 py-4 border-t border-gray-100 dark:border-gray-800 text-center">
                            <button onClick={loadMoreEntries} disabled={loadingMore}
                                ...
                                {tr("seeMoreRemaining", { p1: entryTotal - entries.length })}
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-4">
                    ...
                    {!loading && cbTxns.length > 0 && (
                        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-5 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-xs text-green-600 dark:text-green-400 font-medium">{tr("totalCashbackCredited")}</p>
                                <p className="text-xl font-bold text-green-700 dark:text-green-300">
                                    {totalCredited.toLocaleString(dateLocale)} XOF
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                            <h2 className="font-semibold text-gray-800 dark:text-gray-100">{tr("cashbackHistory")}</h2>
                            <p className="text-xs text-gray-500 mt-0.5">{tr("transaction2", { cbTotal, p2: cbTotal !== 1 ? "s" : "" })}</p>
                        </div>
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-16 text-gray-400">
                                ...
                                <p className="text-sm">{tr("noCashbackMoment")}</p>
                                <p className="text-xs mt-1 text-gray-400">{tr("cashbackWillAppearHere")}</p>
                            </div>
                        ) : (
                            <ul className="divide-y divide-gray-100 dark:divide-gray-800">
                                {cbTxns.map((t) => {
                                    ...
                                    return (
                                        <li key={t.id} className="flex items-center gap-3 px-5 py-3.5">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2">
                                                    <p className="text-sm font-medium text-gray-800 dark:text-gray-100">
                                                        {tr("cashback2", { p1: t.rule?.type === "PERCENTAGE"
                                            ...
                                            <p className="text-sm font-bold text-green-600 dark:text-green-400 tabular-nums shrink-0">
                                                +{parseFloat(t.creditedAmount).toLocaleString(dateLocale)} {t.currencyCode}
                                            </p>
                                        </li>
                                    );
                                })}
                        ...
                        {!loading && cbTxns.length < cbTotal && (
                            <div className="px-5 py-4 border-t border-gray-100 dark:border-gray-800 text-center">
                                <button onClick={loadMoreCb} disabled={loadingMore}
                                    className="text-sm text-blue-600 dark:text-blue-400 hover:underline disabled:opacity-50 flex items-center gap-1.5 mx-auto">
                                    {loadingMore && <Loader2 className="h-3 w-3 animate-spin" />}
                                    {tr("seeMoreRemaining", { p1: cbTotal - cbTxns.length })}
                                </button>
                            </div>
                        )}
                    </div>
