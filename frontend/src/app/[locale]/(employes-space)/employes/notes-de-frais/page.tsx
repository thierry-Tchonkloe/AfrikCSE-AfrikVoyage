...
type Translator = ReturnType<typeof useTranslations<"employee.notesDeFrais">>;
...
const getStatusConfig = (t: Translator): Record<string, { label: string; color: string; bg: string }> => ({
    PENDING:   { label: t("pending"), color: "#f59e0b", bg: "#fffbeb" },
    APPROVED:  { label: t("approved"),   color: "#10b981", bg: "#f0fdf4" },
    REJECTED:  { label: t("rejected"),     color: "#ef4444", bg: "#fef2f2" },
    CANCELLED: { label: t("cancelled"),     color: "#6b7280", bg: "#f9fafb" },
});

const getPaymentMethodLabels = (tr: Translator): Record<string, string> => ({
    card: tr("companyCard"),
...
export default function NotesDeFragsPage() {
    ...
    const tr = useTranslations("employee.notesDeFrais");
    const PAYMENT_METHOD_LABELS = useMemo(() => getPaymentMethodLabels(tr), [tr]);
    const t = useTranslations("employee.notesDeFrais");
    ...
    useEffect(() => {
        ...
        .catch(() => toast.error(t("errorLoadingExpenseReports")))
    ...
    return (
        ...
        <div className="flex items-center justify-between">
            <div>
            <h1 className="text-xl font-bold text-gray-900">{t("expenseReports")}</h1>
            <p className="text-sm text-gray-500">{t("submitTrackReimbursements")}</p>
            </div>
            <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
            { label: t("pending"),     value: totalPending,          color: "#f59e0b", bg: "#fffbeb" },
            { label: t("reimbursed"),      value: `${totalApproved.toLocaleString()} XOF`, color: "#10b981", bg: "#f0fdf4" },
            { label: t("totalSubmitted"),   value: expenses.length,       color: "#3b82f6", bg: "#eff6ff" },
            ].map((s) => (
            <div key={s.label}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <thead>
                ...
                {[tr("title"), tr("destination"), tr("date"), tr("amount"), tr("status"), tr("actions")].map((h) => (
            ...
            <tbody>
                ...
                expenses.map((exp) => {
                    ...
                    <tr key={exp.id} className="border-b border-gray-50 hover:bg-gray-50">
                        ...
                        {new Date(exp.expenseDate ?? exp.createdAt).toLocaleDateString(dateLocale)}
                        </td>
        ...
        {selected && (
            ...
            <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-xl space-y-4">
                ...
                <div>
                    ...
                    {new Date(selected.expenseDate ?? selected.createdAt).toLocaleDateString(dateLocale)}
                    </p>
                </div>
                ...
                <div className="rounded-lg p-3 text-xs" style={{ background: "#fef2f2", color: "#ef4444" }}>
                    {t("rejectionReason", { rejectionNote: selected.rejectionNote })}
                </div>
                )}

                {selected.receipts.length > 0 && (
                <div>
                    <p className="text-xs text-gray-400 mb-1">{t("receipts")}</p>
