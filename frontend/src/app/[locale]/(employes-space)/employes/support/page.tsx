...
export default function SupportPage() {
    ...
    const t = useTranslations("employee.support");
    const { user } = useAuth();
    ...
    useEffect(() => {
        employeeService.getSupportConversation()
            ...
            .catch(() => toast.error(t("errorLoadingSupport")))
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            toast.error(t("errorWhileSending"));
            setInput(content);
        } finally {
            setSending(false);
        }
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-130px)] bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                ...
                <div>
                    <p className="font-semibold text-gray-900 text-sm">{t("afrikcseAfrikvoyageSupport")}</p>
                    <p className="text-xs text-gray-400">{t("teamUsuallyRepliesWithin")}</p>
                </div>
            </div>
