...
type Translator = ReturnType<typeof useTranslations<"employee.layout">>;

type NavItem = { href: string; label: string; icon: LucideIcon };
type NavGroup = { id: string; label: string; icon: LucideIcon; items: NavItem[] };

// Lien racine toujours visible, en dehors des accordéons de module.
const getHomeItem = (t: Translator): NavItem => ({ href: "/employes/dashboard", label: t("dashboard"), icon: LayoutDashboard });
...
function buildNavGroups(t: Translator, hasCSE: boolean, hasVoyage: boolean): NavGroup[] {
    ...
    if (hasVoyage) {
        groups.push({
            ...
            icon: Plane,
            items: [
                { href: "/employes/voyages", label: t("myTrips"), icon: Plane },
                { href: "/employes/reserver", label: t("bookYourTrip"), icon: Calendar },
                { href: "/employes/voyages/groupe", label: t("groupTrips"), icon: Users },
                { href: "/employes/reservations", label: t("myBookings"), icon: CalendarCheck },
                { href: "/employes/notes-de-frais", label: t("expenseReports"), icon: FileText },
            ],
        });
    }
    ...
    if (hasCSE) {
        groups.push({
            ...
            icon: Users,
            items: [
                { href: "/employes/avantages", label: t("myBenefits"), icon: Gift },
                { href: "/employes/famille", label: t("myFamily"), icon: Users },
                { href: "/employes/tickets", label: t("myTickets"), icon: Ticket },
                { href: "/employes/recompenses", label: t("rewards"), icon: Trophy },
                { href: "/employes/communication", label: t("cseCommunication"), icon: MessageSquare },
                { href: "/employes/evenements", label: t("eventsCalendar"), icon: CalendarDays },
                { href: "/employes/evenements/galerie", label: t("photoGallery"), icon: Images },
                { href: "/employes/wallet", label: t("myWallet"), icon: Wallet },
                { href: "/employes/economies", label: t("mySavings"), icon: PiggyBank },
                { href: "/employes/carte-membre", label: t("membershipCard"), icon: CreditCard },
            ],
        });
    }
    ...
    groups.push({
        ...
        icon: LifeBuoy,
        items: [
            { href: "/employes/mes-demandes", label: t("myRequests"), icon: FileClock },
            { href: "/employes/notifications", label: t("notifications"), icon: Bell },
            { href: "/employes/faq", label: t("faq"), icon: HelpCircle },
            { href: "/employes/support", label: t("support"), icon: LifeBuoy },
        ],
    });
...
const getNavBottom = (t: Translator) => ([
    { href: "/employes/profile",    label: t("profile"),     icon: User },
    { href: "/employes/parametres", label: t("settings"),  icon: Settings },
]);

export default function EmployeLayout({ children }: { children: React.ReactNode }) {
    const t = useTranslations("employee.layout");
    const HOME_ITEM = useMemo(() => getHomeItem(t), [t]);
    const NAV_BOTTOM = useMemo(() => getNavBottom(t), [t]);
    ...
    return (
        ...
        >
            ...
            {sidebarOpen ? (
                ...
                <div className="min-w-0">
                    <p className="text-base font-display font-bold truncate" style={{ color: ACCENT }}>{t("afrikWorkspace")}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{t("employeeSpace")}</p>
                </div>
                </div>
            ) : (
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="hidden sm:flex items-center flex-1 max-w-md mx-4">
                <GlobalSearch scope="employee" darkMode={darkMode} placeholder={t("searchTripsExpensesBenefits")} />
            </div>
