...
export default function FaqEmployeePage() {
    const t = useTranslations("employee.faq");
    ...
    const handleVote = async (id: string, helpful: boolean) => {
        ...
        try {
            ...
            toast.success(helpful ? t("markedAsHelpful") : t("thankFeedback"));
        } catch (err) {
            toast.error(getErrorMessage(err, t("errorWhileVoting")));
        } finally {
            setVoting(null);
        }
    };
    ...
    return (
        <div className="space-y-5 max-w-3xl mx-auto">
            {/* En-tête */}
            <div>
                <h1 className="text-xl font-bold text-gray-900">{t("frequentlyAskedQuestions")}</h1>
                <p className="text-sm text-gray-500">{t("quicklyFindAnswersQuestions")}</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-10 text-center text-sm text-gray-400">
                    {search ? t("noResultsSearch") : t("noQuestionsPublishedMoment")}
                </div>
            ) : (
                                ...
                                {isOpen && (
                                    <div className="px-5 pb-5 space-y-4 border-t border-gray-100">
                                        ...
                                        <div className="flex items-center gap-3 pt-1">
                                            <span className="text-xs text-gray-400">{t("didAnswerHelp")}</span>
                                            <button
