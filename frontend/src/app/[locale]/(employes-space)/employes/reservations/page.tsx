...
type Translator = ReturnType<typeof useTranslations<"employee.reservations">>;

const getStatusConfig = (t: Translator): Record<BookingStatus, { label: string; color: string; icon: React.ElementType }> => ({
    PENDING:   { label: t("pending"),  color: "text-amber-600 bg-amber-50 dark:bg-amber-900/20",  icon: Clock },
    CONFIRMED: { label: t("confirmed"),   color: "text-blue-600 bg-blue-50 dark:bg-blue-900/20",    icon: CheckCircle2 },
    COMPLETED: { label: t("completed"),    color: "text-green-600 bg-green-50 dark:bg-green-900/20", icon: CheckCircle2 },
    CANCELLED: { label: t("cancelled"),     color: "text-gray-500 bg-gray-50 dark:bg-gray-800",       icon: XCircle },
    REJECTED:  { label: t("declined"),     color: "text-red-600 bg-red-50 dark:bg-red-900/20",       icon: XCircle },
    NO_SHOW:   { label: t("noShow"),color: "text-orange-600 bg-orange-50 dark:bg-orange-900/20", icon: XCircle },
});

export default function ReservationsPage() {
    const getBookingLabel = useBookingLabel();
    const dateLocale = useDateLocale();
    const t = useTranslations("employee.reservations");
    ...
    const load = useCallback(async (p = 1) => {
        ...
        try {
            ...
            toast.error(getErrorMessage(err, t("errorWhileLoading")));
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleCancel = async (id: string) => {
        ...
        try {
            ...
            toast.error(getErrorMessage(err, t("errorWhileCancelling")));
        }
    };
    ...
    const handleRate = async () => {
        ...
        try {
            ...
            toast.error(getErrorMessage(err, t("errorWhileRating")));
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="p-6 space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("myBookings")}</h1>
            ...
            {loading && bookings.length === 0 ? (
                ...
                <div className="text-center py-16 text-gray-400">
                    ...
                    <p className="text-sm">{t("noBookingsMoment")}</p>
                </div>
            ) : (
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">
                                                {t("pers", { name: b.partner?.name ?? "", numberOfPersons: b.numberOfPersons, p3: new Date(b.bookingDate).toLocaleDateString(dateLocale, {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 w-full max-w-sm shadow-2xl space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="font-semibold text-gray-900 dark:text-white">{t("rateExperience")}</h2>
