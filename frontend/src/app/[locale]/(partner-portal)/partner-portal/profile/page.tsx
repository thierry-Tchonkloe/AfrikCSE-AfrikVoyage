...
export default function PartnerProfilePage() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t("partnerProfile")}</h1>
                <p className="text-xs text-gray-500 mt-0.5">{t("informationVisibleEmployees")}</p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        <button type="button" onClick={() => logoInputRef.current?.click()} disabled={uploadingLogo}
                            ...
                            {partner?.logoUrl ? t("changeLogo") : t("addLogo")}
                        </button>
                ...
                <div className={`space-y-4 ${!isAdmin ? "opacity-60 pointer-events-none" : ""}`}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Field label={t("partnerName")} icon={<Building2 size={14} />}>
                            <input value={form.name ?? ""} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                                className="input-field" placeholder={t("nameEstablishment")} />
                        </Field>
                        <Field label={t("industry")} icon={null}>
                            <select value={form.sector ?? ""} onChange={(e) => setForm((f) => ({ ...f, sector: e.target.value }))}
                                className="input-field">
                                <option value="">{t("choose")}</option>
                                {SECTORS.map((s) => <option key={s} value={s}>{optionLabel(s)}</option>)}
                            </select>
                        </Field>
                    </div>

                    <Field label={t("description")} icon={null}>
                        <textarea value={form.description ?? ""} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
                            rows={3} className="input-field resize-none"
                            placeholder={t("describeBusinessServices")} />
                    </Field>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Field label={t("contactEmail")} icon={<Mail size={14} />}>
                            <input type="email" value={form.contactEmail ?? ""}
                                onChange={(e) => setForm((f) => ({ ...f, contactEmail: e.target.value }))}
                                className="input-field" placeholder="contact@etablissement.com" />
                        </Field>
                        <Field label={t("phone")} icon={<Phone size={14} />}>
                            <input value={form.phone ?? ""}
                                onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))}
                                className="input-field" placeholder={t("text229XxXxXx")} />
                        </Field>
                    </div>

                    <Field label={t("website")} icon={<Globe size={14} />}>
