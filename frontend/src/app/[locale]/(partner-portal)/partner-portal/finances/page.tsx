...
export default function PartnerFinancesPage() {
    ...
    const t = useTranslations("partner.finances");
    ...
    const load = useCallback(async (p = 1) => {
        ...
        try {
            ...
            toast.error(getErrorMessage(err, t("errorLoadingFinances")));
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleRequestPayout = async () => {
        ...
        try {
            ...
            toast.success(t("payoutRequestSent", { p1: formatCurrency(payout.netAmount, currencyCode, dateLocale) }));
            load(1);
        } catch (err) {
            toast.error(getErrorMessage(err, t("errorWhileRequestingPayout")));
        } finally {
            setRequesting(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t("finances")}</h1>
                    <p className="text-xs text-gray-500 mt-0.5">
                        {t("earningsCommissionsCharged")}
                    </p>
                </div>
                {isAdmin && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <KpiCard icon={TrendingUp} color="blue"  label={t("grossEarnings")}
                    value={loading ? "—" : formatCurrency(grossRevenue, currencyCode, dateLocale)} />
                <KpiCard icon={Banknote}   color="amber" label={t("platformCommissions")}
                    value={loading ? "—" : formatCurrency(totalCommissions, currencyCode, dateLocale)} />
                <KpiCard icon={PiggyBank}  color="green" label={t("availableBalancePayOut")}
                    value={loading ? "—" : formatCurrency(netBalance, currencyCode, dateLocale)}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700">
                <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
                    <h2 className="font-semibold text-sm text-gray-900 dark:text-white">{t("commissionHistory")}</h2>
                </div>
                {loading ? (
                    ...
                    <>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="text-left text-xs text-gray-500 border-b border-gray-100 dark:border-gray-700">
                                        <th className="px-5 py-2.5 font-medium">{t("booking")}</th>
                                        <th className="px-5 py-2.5 font-medium">{t("date")}</th>
                                        <th className="px-5 py-2.5 font-medium text-right">{t("grossAmount")}</th>
                                        <th className="px-5 py-2.5 font-medium text-right">{t("commission")}</th>
                                        <th className="px-5 py-2.5 font-medium text-right">{t("net")}</th>
                                    ...
                                    {entries.map((e) => (
                                        <tr key={e.id}>
                                            ...
                                            <td className="px-5 py-3 text-right text-gray-900 dark:text-white">
                                                {formatCurrency(e.grossAmount, e.currencyCode, dateLocale)}
                                            </td>
                                            <td className="px-5 py-3 text-right text-red-600">
                                                -{formatCurrency(e.commissionAmount, e.currencyCode, dateLocale)}
                                            </td>
                                            <td className="px-5 py-3 text-right font-medium text-gray-900 dark:text-white">
                                                {formatCurrency(e.netAmount, e.currencyCode, dateLocale)}
                                            </td>
                                            <td className="px-5 py-3">
                                                <span data-testid="payout-status" className={`text-xs px-2 py-0.5 rounded-full ${
                                                    ...
                                                    {e.payoutStatus === "CLAIMED" ? t("claimed") : t("available")}
                                                </span>
                                            </td>
                                        </tr>
                                    ))}
                        ...
                        {totalPages > 1 && (
                            <div className="flex items-center justify-center gap-2 py-4 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <span className="text-xs text-gray-500">{t("page", { page, totalPages })}</span>
