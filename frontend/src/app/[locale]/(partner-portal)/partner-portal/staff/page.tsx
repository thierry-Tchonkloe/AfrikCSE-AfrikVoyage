...
export default function PartnerStaffPage() {
    ...
    const handleInvite = async () => {
        if (!form.email.trim() || !form.firstName.trim() || !form.lastName.trim() || form.password.length < 8) {
            toast.error(t("allFieldsRequiredPassword"));
            return;
        }
        setSaving(true);
        try {
            ...
            setShowModal(false);
            setForm(EMPTY_FORM);
            toast.success(t("memberInvitedSuccessfully"));
        } catch (err) {
            toast.error(getErrorMessage(err, t("errorWhileInviting")));
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDeactivate = async (member: PartnerUser) => {
        if (!confirm(t("deactivate", { firstName: member.firstName, lastName: member.lastName }))) return;
    ...
    return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t("team")}</h1>
                    <p className="text-xs text-gray-500 mt-0.5">{t("activeMember", { length: active.length, p2: active.length !== 1 ? "s" : "" })}</p>
                </div>
                {isAdmin && (
            ...
            {!isAdmin && (
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3 text-sm text-amber-700 dark:text-amber-400">
                    {t("onlyPartnerAdministratorsCan")}
                </div>
            )}
                ...
                <>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2">
                            ...
                            <h2 className="font-semibold text-sm text-gray-900 dark:text-white">{t("activeMembers")}</h2>
                        </div>
                        {active.length === 0 ? (
                    ...
                    {inactive.length > 0 && (
                        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden opacity-70">
                            <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
                                <h2 className="font-semibold text-sm text-gray-500">{t("deactivatedMembers", { length: inactive.length })}</h2>
                            </div>
                            <div className="divide-y divide-gray-100 dark:divide-gray-700">
                                {inactive.map((member) => (
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
                    <div className="w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="font-bold text-gray-900 dark:text-white">{t("inviteMember")}</h2>
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                <F label={t("firstName")}>
                                    <input value={form.firstName} onChange={(e) => setForm((f) => ({ ...f, firstName: e.target.value }))}
                                        className="inp" placeholder={t("john")} />
                                </F>
                                <F label={t("lastName")}>
                                    <input value={form.lastName} onChange={(e) => setForm((f) => ({ ...f, lastName: e.target.value }))}
                                        className="inp" placeholder={t("doe")} />
                                </F>
                            </div>
                            <F label={t("email")}>
                            ...
                            <F label={t("temporaryPassword")}>
                                <input type="password" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))}
                                    className="inp" placeholder={t("min8Characters")} />
                            </F>
                            <p className="text-xs text-gray-400">{t.rich("memberWillCreatedStaff", { strong1: (chunks) => <strong>{chunks}</strong> })}</p>
                        </div>
                        <div className="flex justify-end gap-2 pt-2">
                            ...
                            <button onClick={handleInvite} disabled={saving}
                                ...
                                {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : t("invite")}
                            </button>
                        </div>
                    </div>
                </div>
