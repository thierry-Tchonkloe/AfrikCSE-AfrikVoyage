...
type Translator = ReturnType<typeof useTranslations<"partner.layout">>;

const getNavItems = (t: Translator) => ([
    { href: "/partner-portal/dashboard",  label: t("dashboard"),  icon: LayoutDashboard },
    { href: "/partner-portal/bookings",   label: t("bookings"),     icon: CalendarCheck },
    { href: "/partner-portal/finances",   label: t("finances"),         icon: Wallet },
    { href: "/partner-portal/offers",     label: t("offers"),           icon: Layers },
    { href: "/partner-portal/locations",  label: t("locations"),   icon: MapPin },
    { href: "/partner-portal/staff",      label: t("team"),           icon: Users },
    { href: "/partner-portal/profile",    label: t("profile"),           icon: Building2 },
    { href: "/partner-portal/settings",   label: t("settings"),       icon: Settings },
]);

export default function PartnerPortalLayout({ children }: { children: React.ReactNode }) {
    const t = useTranslations("partner.layout");
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-gray-50 dark:bg-gray-900">
            ...
            <motion.aside
                ...
                className={cn(
                    "fixed lg:static inset-y-0 left-0 z-30 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 border-r overflow-hidden",
                    open ? "translate-x-0" : "-translate-x-full",
                    "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700"
                )}
            >
                {/* Logo */}
                <div className="flex items-center h-16 px-3 gap-2 border-b border-gray-200 dark:border-gray-700 shrink-0">
                    {open ? (
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{t("partnerSpace")}</p>
                            </div>
                        </div>
                    ) : (
                ...
                <div className="p-3 border-t border-gray-200 dark:border-gray-700 shrink-0">
                    {open ? (
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={logout} title={t("logOut")} className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400">
                                <LogOut size={14} />
                            </button>
                        </div>
                    ) : (
                        <button onClick={logout} title={t("logOut")} className="w-full flex justify-center p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400">
                            <LogOut size={16} />
                        </button>
                    )}
                </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                <header className="h-16 flex items-center justify-between px-6 border-b bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm font-semibold text-gray-900 dark:text-white">
                            {NAV_ITEMS.find((n) => pathname.startsWith(n.href))?.label ?? t("partnerPortal")}
                        </p>
                    </div>

                    {/* Identité du partenaire connecté */}
                    <div className="flex items-center gap-2 min-w-0">
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate max-w-[160px] sm:max-w-xs">
                            {user.partnerName}
                        </span>
                        {user.partnerLogoUrl ? (
                            <Image src={user.partnerLogoUrl} alt={user.partnerName} width={28} height={28} className="w-7 h-7 rounded-full object-cover shrink-0" />
                        ) : (
                ...
                <footer className="py-3 px-6 text-center text-xs text-gray-400 border-t border-gray-100 dark:border-gray-800 shrink-0">
                    {t("allRightsReservedWaxeho")}
                </footer>
            </div>
        </div>
    );
