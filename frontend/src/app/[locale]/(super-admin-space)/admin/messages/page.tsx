...
type Translator = ReturnType<typeof useTranslations<"admin.messages">>;
...
const getStatusConfig = (t: Translator): Record<ConvStatus, { label: string; color: string }> => ({
    OPEN:     { label: t("open"), color: "#f59e0b" },
...
export default function AdminMessagesPage() {
    ...
    const handleToggleStatus = async () => {
        ...
        try {
            ...
            toast.success(
                newStatus === "RESOLVED" ? t("conversationMarkedAsResolved") : t("conversationReopened")
            );
        } catch {
    ...
    const getOrgName = (conv: Conversation): string => conv.organization?.name ?? t("unknownOrganization");
    ...
    const getSenderLabel = (sender: Message["sender"]): string =>
        sender.role === "SUPER_ADMIN" ? t("afrikcseSupport") : `${sender.firstName} ${sender.lastName}`;
        ...
        <div className="flex h-[calc(100vh-130px)] bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="w-72 flex flex-col border-r border-gray-200 shrink-0">
                ...
                <div className="flex items-center justify-between p-4 border-b border-gray-100">
                    <h2 className="font-bold text-gray-900">{t("messagesCompanies")}</h2>
                </div>

                {/* Filtres statut */}
                <div className="flex gap-1.5 px-3 pt-3">
                    {[
                        { key: "ALL" as const, label: t("all"), count: totalCount },
                        { key: "OPEN" as const, label: t("open2"), count: counts.open },
                        { key: "RESOLVED" as const, label: t("resolved2"), count: counts.resolved },
                    ].map((tab) => (
                        <button
                            key={tab.key}
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-center gap-1">
                                        ...
                                        {lastMsg && (
                                            <span className="text-xs text-gray-400 shrink-0">
                                                {new Date(lastMsg.createdAt).toLocaleTimeString(dateLocale, {
                                    ...
                                    <div className="flex items-center justify-between gap-1 mt-0.5">
                                        {lastMsg ? (
                                            ...
                                            <p className="text-xs text-gray-400 italic">{t("noMessages")}</p>
                                        )}
                                        {conv.unreadCount > 0 && (
                                            <span
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    ...
                    <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className={`text-xs ${isOrgOnline(activeConv) ? "text-green-500" : "text-gray-400"}`}>
                                    {isOrgOnline(activeConv) ? t("online") : t("offline")}
                                </p>
                            </div>
                        </div>
                        ...
                        >
                            {activeConv.status === "OPEN" ? (
                                <>
                                    <CheckCircle2 size={14} /> {t("markAsResolved")}
                                </>
                            ) : (
                                <>
                    ...
                    <div className="px-6 py-4 border-t border-gray-100">
                        <div className="flex items-center gap-3">
                            <input
                                ...
                                placeholder={t("replyAsAfrikcseSupport")}
