...
function PartnerSelect({ value, onChange, partners }: { value: string; onChange: (v: string) => void; partners: Partner[] }) {
    const t = useTranslations("admin.travelCatalog");
    return (
        <div>
            <label className="text-xs font-medium text-gray-600">{t("partner")}</label>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
                <div className="flex justify-between items-center">
                    <h2 className="font-bold text-gray-900">{isEdit ? t("edit") : t("add")} {t("airport")}</h2>
                    <button type="button" onClick={onClose}><X size={18} /></button>
                </div>
                {error && <p className="text-sm text-red-600">{error}</p>}
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("iataCode")}</label>
                    ...
                    <div className="col-span-2">
                        <label className="text-xs font-medium text-gray-600">{t("airportName")}</label>
                ...
                <div className="flex justify-end gap-2">
                    <button type="button" onClick={onClose} className="text-sm border px-4 py-1.5 rounded hover:bg-gray-50">{t("cancel")}</button>
                    <button type="submit" className="text-sm bg-blue-600 text-white px-4 py-1.5 rounded hover:bg-blue-700">{t("save")}</button>
                </div>
            </form>
        </div>
    );
...
function AirportsTab() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm(t("deleteAirport"))) return;
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-base font-semibold text-gray-700">{t("airports", { length: airports.length })}</h2>
                ...
                <div className="bg-white rounded-xl border overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b text-gray-500 text-xs">
                            <tr><th className="text-left px-4 py-3">{t("code")}</th><th className="text-left px-4 py-3">{t("name")}</th><th className="text-left px-4 py-3">{t("city2")}</th><th className="text-left px-4 py-3">{t("country2")}</th><th className="text-left px-4 py-3">{t("actions")}</th></tr>
                        </thead>
                        <tbody>
                            {airports.length === 0 ? (
                                <tr><td colSpan={5} className="text-center py-8 text-gray-400">{t("noAirport")}</td></tr>
                            ) : airports.map((a) => (
    ...
    return (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center">
                    <h2 className="font-bold text-gray-900">{isEdit ? t("edit") : t("add")} {t("airRoute")}</h2>
                    <button type="button" onClick={onClose}><X size={18} /></button>
                </div>
                {error && <p className="text-sm text-red-600">{error}</p>}
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("airlineCode")}</label>
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("departureTime")}</label>
                        <input type="time" value={form.departureTime} onChange={(e) => setForm((f) => ({ ...f, departureTime: e.target.value }))}
                            required className="w-full border rounded px-3 py-1.5 text-sm mt-1" />
                    </div>
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("originAirportIata")}</label>
                        <input value={form.originIata} onChange={(e) => setForm((f) => ({ ...f, originIata: e.target.value }))}
                            required maxLength={3} placeholder="LOS" className="w-full border rounded px-3 py-1.5 text-sm mt-1" />
                    </div>
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("originCity")}</label>
                        <input value={form.originCity} onChange={(e) => setForm((f) => ({ ...f, originCity: e.target.value }))}
                            required className="w-full border rounded px-3 py-1.5 text-sm mt-1" />
                    </div>
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("destinationAirportIata")}</label>
...
function FlightsTab() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm(t("deleteRoute"))) return;
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-base font-semibold text-gray-700">{t("airRoutes", { length: routes.length })}</h2>
                <button onClick={() => setModal("create")} className="flex items-center gap-1 text-sm bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700">
                    <Plus size={14} /> {t("add")}
                </button>
            </div>
            {loading ? <p className="text-sm text-gray-500">{t("loading")}</p> : (
                <div className="bg-white rounded-xl border overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b text-gray-500 text-xs">
                            <tr>
                                <th className="text-left px-4 py-3">{t("airline")}</th><th className="text-left px-4 py-3">{t("route")}</th>
                                <th className="text-left px-4 py-3">{t("departure")}</th><th className="text-left px-4 py-3">{t("duration")}</th>
                                <th className="text-left px-4 py-3">{t("price")}</th><th className="text-left px-4 py-3">{t("active")}</th><th className="text-left px-4 py-3">{t("actions")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {routes.length === 0 ? (
                                <tr><td colSpan={7} className="text-center py-8 text-gray-400">{t("noRoute")}</td></tr>
                            ) : routes.map((r) => (
                                <tr key={r.id} className="border-t hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-gray-600">{t("min", { durationMinutes: r.durationMinutes })}</td>
...
function HotelPropertyModal({ initial, partners, onClose, onSaved }: {
    ...
    return (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
                <div className="flex justify-between items-center">
                    <h2 className="font-bold text-gray-900">{isEdit ? t("edit") : t("add")} {t("hotel")}</h2>
                    <button type="button" onClick={onClose}><X size={18} /></button>
                </div>
                {error && <p className="text-sm text-red-600">{error}</p>}
                <PartnerSelect value={form.partnerId} onChange={(v) => setForm((f) => ({ ...f, partnerId: v }))} partners={partners} />
                <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                        <label className="text-xs font-medium text-gray-600">{t("name2")}</label>
                        <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                            required className="w-full border rounded px-3 py-1.5 text-sm mt-1" />
                    </div>
                    <div>
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("stars")}</label>
...
function RoomTypeRow({ room, onChanged }: { room: HotelRoomType; onChanged: () => void }) {
    ...
    const handleDelete = async () => {
        if (!confirm(t("deleteRoomType"))) return;
    ...
    return (
        <tr className="border-t border-gray-100">
            ...
            <td className="px-3 py-2">{t("pers", { capacity: room.capacity })}</td>
...
function AddRoomTypeForm({ hotelId, onAdded }: { hotelId: string; onAdded: () => void }) {
    ...
    return (
        <tr className="border-t border-gray-100 bg-gray-50">
            <td className="px-3 py-2"><input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder={t("standardRoom")} className="w-full border rounded px-2 py-1 text-xs" /></td>
            ...
            <td className="px-3 py-2"><button onClick={handleAdd} className="text-blue-600 hover:text-blue-800 text-xs font-medium">{t("add2")}</button></td>
        </tr>
    );
}
...
function HotelsTab() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm(t("deleteHotelItsRoom"))) return;
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-base font-semibold text-gray-700">{t("hotels", { length: hotels.length })}</h2>
                <button onClick={() => setModal("create")} className="flex items-center gap-1 text-sm bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700">
                    <Plus size={14} /> {t("add")}
                </button>
            </div>
            {loading ? <p className="text-sm text-gray-500">{t("loading")}</p> : hotels.length === 0 ? (
                <p className="text-sm text-gray-500">{t("noHotel")}</p>
            ) : (
                            ...
                            <div className="flex justify-between items-center p-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => setExpanded(expanded === h.id ? null : h.id)} className="text-xs flex items-center gap-1 text-gray-600 border px-2 py-1 rounded hover:bg-gray-50">
                                        {expanded === h.id ? <ChevronUp size={12} /> : <ChevronDown size={12} />} {t("rooms", { length: h.roomTypes.length })}
                                    </button>
                            ...
                            {expanded === h.id && (
                                <div className="border-t bg-gray-50 px-4 pb-4">
                                    <table className="w-full text-xs mt-2">
                                        <thead><tr className="text-gray-500"><th className="text-left px-3 py-1">{t("name")}</th><th className="text-left px-3 py-1">{t("capacity")}</th><th className="text-left px-3 py-1">{t("priceNight")}</th><th className="text-left px-3 py-1">{t("rooms2")}</th><th></th></tr></thead>
                                        <tbody>
...
function TrainRouteModal({ initial, partners, onClose, onSaved }: {
    ...
    return (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center">
                    <h2 className="font-bold text-gray-900">{isEdit ? t("edit") : t("add")} {t("trainRoute")}</h2>
                    <button type="button" onClick={onClose}><X size={18} /></button>
                </div>
                {error && <p className="text-sm text-red-600">{error}</p>}
                <PartnerSelect value={form.partnerId} onChange={(v) => setForm((f) => ({ ...f, partnerId: v }))} partners={partners} />
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("originStation")}</label>
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("destinationStation")}</label>
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("class")}</label>
...
function TrainsTab() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm(t("deleteRoute2"))) return;
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-base font-semibold text-gray-700">{t("trainRoutes", { length: routes.length })}</h2>
                <button onClick={() => setModal("create")} className="flex items-center gap-1 text-sm bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700">
                    <Plus size={14} /> {t("add")}
                </button>
            </div>
            {loading ? <p className="text-sm text-gray-500">{t("loading")}</p> : (
                <div className="bg-white rounded-xl border overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b text-gray-500 text-xs">
                            <tr>
                                <th className="text-left px-4 py-3">{t("operator")}</th><th className="text-left px-4 py-3">{t("route")}</th>
                                <th className="text-left px-4 py-3">{t("class")}</th><th className="text-left px-4 py-3">{t("price")}</th>
                                <th className="text-left px-4 py-3">{t("active")}</th><th className="text-left px-4 py-3">{t("actions")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {routes.length === 0 ? (
                                <tr><td colSpan={6} className="text-center py-8 text-gray-400">{t("noRoute2")}</td></tr>
                            ) : routes.map((r) => (
                                <tr key={r.id} className="border-t hover:bg-gray-50">
...
function CarRentalVehicleModal({ initial, partners, onClose, onSaved }: {
    ...
    return (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center">
                    <h2 className="font-bold text-gray-900">{isEdit ? t("edit") : t("add")} {t("vehicle")}</h2>
                    <button type="button" onClick={onClose}><X size={18} /></button>
                </div>
                {error && <p className="text-sm text-red-600">{error}</p>}
                <PartnerSelect value={form.partnerId} onChange={(v) => setForm((f) => ({ ...f, partnerId: v }))} partners={partners} />
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("transmission")}</label>
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("seats")}</label>
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("pricePerDay")}</label>
...
function CarsTab() {
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-base font-semibold text-gray-700">{t("vehicles", { length: vehicles.length })}</h2>
                <button onClick={() => setModal("create")} className="flex items-center gap-1 text-sm bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700">
                    <Plus size={14} /> {t("add")}
                </button>
            </div>
            {loading ? <p className="text-sm text-gray-500">{t("loading")}</p> : (
                <div className="bg-white rounded-xl border overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b text-gray-500 text-xs">
                            <tr>
                                <th className="text-left px-4 py-3">{t("agency")}</th><th className="text-left px-4 py-3">{t("vehicle2")}</th>
                                <th className="text-left px-4 py-3">{t("city2")}</th><th className="text-left px-4 py-3">{t("priceDay")}</th>
                                <th className="text-left px-4 py-3">{t("active")}</th><th className="text-left px-4 py-3">{t("actions")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {vehicles.length === 0 ? (
                                <tr><td colSpan={6} className="text-center py-8 text-gray-400">{t("noVehicle")}</td></tr>
                            ) : vehicles.map((v) => (
...
export default function TravelCatalogPage() {
    ...
    const TABS: { id: TabId; label: string; icon: React.ElementType }[] = [
        ...
        { id: "hotels",   label: t("hotels2"),                icon: Hotel },
        { id: "trains",   label: t("trains"),                icon: Train },
        { id: "cars",     label: t("carRental"),  icon: Car },
        { id: "airports", label: t("airports2"),              icon: MapPin },
    ];
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h1 className="text-xl font-bold text-gray-900">{t("travelCatalog")}</h1>
                    <p className="text-sm text-gray-500">{t("flightsHotelsTrainsVehicle")}</p>
                </div>
            </div>
