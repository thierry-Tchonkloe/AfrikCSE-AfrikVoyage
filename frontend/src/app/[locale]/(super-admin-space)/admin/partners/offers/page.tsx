...
type Translator = ReturnType<typeof useTranslations<"admin.partnersOffers">>;

const getOfferTypeLabels = (t: Translator): Record<string, string> => ({
    VOUCHER: t("voucher"),
    BOOKING: t("booking"),
...
export default function PendingPartnerOffersPage() {
    ...
    const t = useTranslations("admin.partnersOffers");
    const OFFER_TYPE_LABELS = useMemo(() => getOfferTypeLabels(t), [t]);
    ...
    const handleApprove = async (offer: CatalogItem) => {
        ...
        try {
            ...
            toast.success(t("offerApprovedVisible"));
    ...
    return (
        <div className="space-y-5">
            <div>
                <h1 className="text-xl font-bold text-gray-900">{t("partnerOffersApprove")}</h1>
                <p className="text-sm text-gray-500">
                    {t("offerCreatedModifiedPartner")}
                </p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                    ...
                    <p className="text-gray-500 font-medium">{t("noPendingOffers")}</p>
                    <p className="text-xs text-gray-400 mt-1">{t("allOffersSubmittedPartners")}</p>
                </div>
            ) : (
                        ...
                        <div key={o.id} className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    >
                                        {o.partner?.name ?? t("unknownPartner")}
                                    </button>
                                </div>
                            ...
                            <div className="flex items-center justify-between text-sm pt-2 border-t border-gray-100">
                                <p className="font-bold text-gray-900">
                                    {fmt(o.employeePrice)} <span className="text-xs font-normal text-gray-400">{t("xofPerUnit")}</span>
                                </p>
                            ...
                            <div className="flex gap-2 pt-1">
                                ...
                                >
                                    {processing === o.id ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} {t("approve")}
                                </button>
                                <button
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-xl max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <button
                                        ...
                                        className="text-xs text-gray-500 hover:underline truncate"
                                    >
                                        {detailTarget.partner?.name ?? t("unknownPartner")}
                                    </button>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                            <DetailRow label={t("employeePrice")} value={`${fmt(detailTarget.employeePrice)} XOF`} />
                            <DetailRow label={t("companyPrice")} value={`${fmt(detailTarget.companyPrice)} XOF`} />
                            <DetailRow label={t("category")} value={detailTarget.category} />
                            <DetailRow label={t("offerType")} value={OFFER_TYPE_LABELS[detailTarget.offerType] ?? detailTarget.offerType} />
                            <DetailRow label={t("subsidy")} value={detailTarget.subsidyAmount ? `${fmt(detailTarget.subsidyAmount)} XOF` : `${detailTarget.subsidyPct} %`} />
                            {detailTarget.stock != null && <DetailRow label={t("stock")} value={String(detailTarget.stock)} icon={<Package size={12} />} />}
                            {detailTarget.validUntil && <DetailRow label={t("validUntil")} value={new Date(detailTarget.validUntil).toLocaleDateString(dateLocale)} icon={<CalendarClock size={12} />} />}
                            {(detailTarget.city || detailTarget.region || detailTarget.country) && (
                                <DetailRow label={t("location")} value={[detailTarget.city, detailTarget.region, detailTarget.country].filter(Boolean).join(", ")} icon={<MapPin size={12} />} />
                            )}
                            {detailTarget.requiresTicket && <DetailRow label={t("ticketRequired")} value={t("yes")} icon={<Ticket size={12} />} />}
                            {detailTarget.requiresFamilyMember && <DetailRow label={t("familyMemberRequired")} value={t("yes")} icon={<Users size={12} />} />}
                            {detailTarget.isFeatured && <DetailRow label={t("featured")} value={t("yes")} icon={<Star size={12} />} />}
                        </div>

                        <p className="text-xs text-gray-400 mb-4">
                            {t("submitted", { p1: new Date(detailTarget.createdAt).toLocaleDateString(dateLocale) })}
                        </p>
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                style={{ background: "#10b981" }}
                            >
                                {processing === detailTarget.id ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} {t("approve")}
                            </button>
                            <button
            ...
            {rejectTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-xl">
                        <div className="flex justify-between items-start mb-4">
                            <h3 className="font-bold text-gray-900">{t("reject2", { title: rejectTarget.title })}</h3>
                        ...
                        <textarea
                            ...
                            rows={4}
                            placeholder={t("eGInconsistentPrice")}
