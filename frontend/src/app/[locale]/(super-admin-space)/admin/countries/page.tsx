        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
                <div className="flex justify-between items-center">
                    <h2 className="font-bold text-gray-900">{isEdit ? t("edit") : t("add")} {t("country")}</h2>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("isoCode")}</label>
                    ...
                    <div>
                        ...
                        <input value={form.name} onChange={e => set("name", e.target.value)}
                            required placeholder={t("ivoryCoast")}
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("locale")}</label>
                        <input value={form.locale} onChange={e => set("locale", e.target.value)}
                            placeholder="fr-CI"
                            className="w-full border rounded px-3 py-1.5 text-sm mt-1" />
                    </div>
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("taxRate01")}</label>
                    ...
                    <div>
                        <label className="text-xs font-medium text-gray-600">{t("phonePrefix")}</label>
...
export default function CountriesPage() {
    ...
    async function handleDelete(code: string) {
        if (!confirm(t("deleteCountry", { code }))) return;
        ...
        <div className="p-6 max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-gray-900">{t("countriesCurrencies")}</h1>
                        <p className="text-sm text-gray-500">{t("configurationCountries")}</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b text-gray-500 text-xs">
                            <tr>
                                <th className="text-left px-4 py-3">{t("code")}</th>
                                <th className="text-left px-4 py-3">{t("name2")}</th>
                                <th className="text-left px-4 py-3">{t("currency")}</th>
                                <th className="text-left px-4 py-3">{t("locale")}</th>
                                <th className="text-left px-4 py-3">{t("tax")}</th>
                                <th className="text-left px-4 py-3">{t("prefix")}</th>
                        ...
                        <tbody>
                            {countries.length === 0 ? (
                                <tr><td colSpan={8} className="text-center py-8 text-gray-400">{t("noCountryConfigured")}</td></tr>
                            ) : countries.map(c => (
